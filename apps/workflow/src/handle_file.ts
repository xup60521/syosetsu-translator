...
async function refreshAccessToken(refreshToken: string) {
  ...
  if (!res.ok) {
    ...
    throw new Error(
      `Failed to refresh access token: ${res.status} ${res.statusText} ${text}`,
    );
  }
...
async function driveListFiles(accessToken: string, q: string, fields: string) {
  ...
  return driveFetch<{ files?: Array<{ id?: string }> }>(
    accessToken,
    url.toString(),
    { method: "GET" },
  );
...
export async function handle_file(data: HandleFileInput) {
  const {
